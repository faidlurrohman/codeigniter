				...
				<div class="col-sm-8 main_blog">
					<div id="carouselTransport" class="carousel slide" data-ride="carousel">
					  	...
					  	<div class="carousel-inner">
				           	<?php
				           		$i = 0;
								foreach($imgSlider->result() as $row){
									if($i == 0){
				    		?>					
							    	<div class="item active">
							      		<img src="<?php echo base_url('assets/dub_user/images/paket/'); ?><?= $row->img ?>">
							    	</div>
						    <?php
									}
									else{
					...
					<div class="comment_area">
						<h3><?= $paketDetail->nama ?></h3>
						<div class="media">
							<div class="media-body">
								<h5><i class="fa fa-clock-o" aria-hidden="true" style="color: #f6b60b;"></i>&nbsp;&nbsp;Durasi :</h5>
								<p><?= $paketDetail->durasi ?></p>
								<h5><i class="fa fa-tag" aria-hidden="true" style="color: #f6b60b;"></i>&nbsp;&nbsp;Harga :</h5>
								<p>Rp. <?= number_format($paketDetail->harga) ?></p>
							</div>
						</div>
					</div>
					...
					<div class="resent">
						<?php 
							foreach($dataTour->result() as $row){
								if($row->id_tour == $paketDetail->id_tour){
						 ?>
									<h3 style="text-transform: uppercase;"><?= $row->kategori ?></h3>
						<?php 
								}
							}
							$jmlFor = 0;
							foreach(array_reverse($semuaPaket->result()) as $row){
								if($row->id_tour == $paketDetail->id_tour){
						 ?>
							<div class="media">
								<div class="media-left">
									<a href="<?php echo base_url(); ?>paket/<?= $row->id_paket ?>">

							           	<?php 
											foreach($imgPaket->result() as $rowImg){
												if($rowImg->id_paket == $row->id_paket){
							    		?>				
													<img class="media-object" src="<?php echo base_url('assets/dub_user/images/paket/'); ?><?= $rowImg->img ?>" style="width: 65px !important; height: 40px !important;">
										<?php
													break;
												}
											}
								...
								<div class="media-body">
									<a style="text-transform: capitalize;" href="<?php echo base_url(); ?>paket/<?= $row->id_paket ?>"><b><?= $row->nama ?></b></a>
									<h6 style="text-transform: capitalize;"><i class="fa fa-clock-o" aria-hidden="true" style="color: #f6b60b;"></i>&nbsp;&nbsp;Durasi : <?= $row->durasi ?></h6>
								</div>
							</div>
						<?php 
								}
